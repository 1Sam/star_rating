jQuery(document).ready(function(){
	...
	jQuery('.rating i,.rating a,.rating img').hover(function(){
		var uid = this.id;
		var uids = uid.split("_");

		jQuery('#rateit_'+uids[1]).css({visibility: "hidden"});
		for(var x=1;x<=uids[0];x++){jQuery('#'+x+'_'+uids[1]).css('color', '#E9D801');}
	...
	jQuery('.rating i').mouseout(function() {
		...
		var uids = uid.split("_");
		jQuery('#rateit_'+uids[1]).css({visibility: "hidden"});
		for(var x=1;x<=uids[0];x++){jQuery('#'+x+'_'+uids[1]).css('color', 'gray');}
	})
...
function callback_add_default(result){
	...
	if(result['message'] == "success"){
		...
		var star_html_code = get_star_html_default(result['star_max'],result['converted_average'],result['document_srl']);
		...
		jQuery("#ment_"+result['document_srl']).html('<i class="fa fa-user"></i>:');
		...
		jQuery("#already_"+result['document_srl']).css({visibility: "visible", "font-size":"12px",color: "red","padding-left":"15px"});
...
function callback_remove_default(result){
	...
	if(result['message'] == 'success'){
		//별코드
		var star_html_code = get_star_html_code(result['star_max'], result['converted_average'],result['document_srl']);

		jQuery("#final_"+result['document_srl']).html(star_html_code);

		jQuery("#ment_"+result['document_srl']).html('Average rating:');
