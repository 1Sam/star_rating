jQuery(document).ready(function(){
	...
	jQuery('.rating i,.rating a,.rating img').hover(function(){
		...
		for(var x=1;x<=uids[0];x++){jQuery('#'+x+'_'+uids[1]).css('backgroundcolor', '#E9D801');}
...
// 별점 부과
function callback_add_star_image(result) {
	//alert(result['error']+'/'+result['message']+'/'+result['html_code']);
	//alert(result['star_max'] + '//' + result['converted_average'] + '//' + result['average']);
	...
	if(result['message'] == "success"){
		var star_html_code = get_star_html_star_image(result['star_max'],result['converted_average'],result['document_srl']);
...
// 별점 삭제
function callback_remove_star_image(result){
	//alert(result['error']+'/'+result['message']+'/'+result['html_code']);
	//alert(result['star_max'] + '//' + result['converted_average'] + '//' + result['average']);
...
function get_star_html_star_image(star_max, star_percent_average, document_srl){
	var html_code ='<div class="final" id="final_'+document_srl+'">';
	for(i=1;i<=star_max;i++) {
	   	if(star_percent_average>=1)	{
			html_code += '<img src="./widgets/star_rating/skins/star_image/images/star_full.gif">';
	        star_percent_average-=1;
        } else if(star_percent_average>=0.5) {
			html_code += '<img src="./widgets/star_rating/skins/star_image/images/star_half.gif"></i>';
			star_percent_average-=1;
       	} else if (star_percent_average<0.5 && star_percent_average>0) {
            html_code += '<img src="./widgets/star_rating/skins/star_image/images/star_blank.gif">';
   	   	    star_percent_average-=1;
